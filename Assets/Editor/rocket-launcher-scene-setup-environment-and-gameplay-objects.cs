...
public partial class SceneSetupTool
{
    ...
    private static readonly float CamBottom = CamY - CamOrthoSize;   // -7
    private static readonly float CamTop    = CamY + CamOrthoSize;   // 11

    // Step 1b: Camera defines visible X range (iPhone 15 Pro Max aspect 19.5:9)
    private const float TargetAspect   = 9f / 19.5f;                          // ~0.4615
    private static readonly float CamHalfWidth = CamOrthoSize * TargetAspect; // ~4.154
    private static readonly float CamLeft      = -CamHalfWidth;               // ~-4.154
    ...
    // Step 2b: Target position — 4 screen-widths right of center
    private static readonly float TargetX = CamHalfWidth * 4f;  // ~16.6

    // Step 2c: Ground — extremely wide, centered at 0
    private const float GroundWidth   = 500f;
    private const float GroundCenterX = 0f;

    // Step 3: Vehicle sits ON ground (wheel bottoms touch ground top)
    private const float WheelLocalY  = -0.5f;   // wheel offset below vehicle pivot
    private const float WheelRadius  = 0.2f;    // half of wheel scale (0.4)
    // Wheel center world = VehicleY + WheelLocalY
    // Wheel bottom world = wheel center - WheelRadius
    // Want: wheel bottom = GroundTop
    // So: VehicleY + WheelLocalY - WheelRadius = GroundTop
    // VehicleY = GroundTop - WheelLocalY + WheelRadius = 1.0 + 0.5 + 0.2 = 1.7
    private static readonly float VehicleY = GroundTop - WheelLocalY + WheelRadius;  // 1.7
    private const float VehicleOffsetFromLeft = 2.5f;                                // padding from left camera edge
    private static readonly float VehicleX   = CamLeft + VehicleOffsetFromLeft;      // ~-1.654
    ...
    // Step 5: Target floats in the sky — midway between ground top and camera top
    private const float TargetScaleY = 1.5f;
    private static readonly float TargetY = (GroundTop + CamTop) / 2f;  // ~5, in the sky

    private static void CreateEnvironment(GameObject parent)
    {
        // Ground — brown strip, bottom edge = camera bottom, extends left-to-right past all views
        ...
        // Sits on ground top: center Y = GroundTop + TargetScaleY/2 = 1.75
    ...
    private static void CreateLauncherVehicle(GameObject parent)
    {
        ...
        // Rocket spawn point — on top of cabin
    ...
    private static void CreateRocket(GameObject parent)
    {
        ...
        // Nose cone — rotated diamond on top
